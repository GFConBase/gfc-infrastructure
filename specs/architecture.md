...
These principles are derived from and consistent with:
...
Once the softcap is reached, the contract allows withdrawals according to predefined rules, but does not mandate timing or purpose beyond the presale scope.
...
Non-breaking clarifications may occur.
Breaking changes are exceptional and must be explicitly documented.
